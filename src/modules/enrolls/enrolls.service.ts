...
export class EnrollsService {
  ...
  studentEnrollChangeStatus = async (
    courseId: string,
    ...
    try {
      ...
      const enroll = await this.enrollRepository
        .createQueryBuilder('enroll')
        .leftJoinAndSelect('enroll.course', 'course')
        .where('enroll.student.id = :id', { id: user.id })
  ...
  studentGetEnrollsCourses = async (student: AuthUserType) => {
    try {
      ...
      const enrolls = await this.enrollRepository
        .createQueryBuilder('enroll')
        .leftJoinAndSelect('enroll.course', 'course')
        .where('enroll.student.id = :id', { id: user.id })
        .getMany();
  ...
  instructorGetEnrollsCourses = async (instructor: AuthUserType) => {
    try {
      ...
      const enrolls = await this.enrollRepository
        .createQueryBuilder('enroll')
        .leftJoinAndSelect('enroll.course', 'course')
        .where('course.creator.id = :id', { id: user.id })
        .getMany();
      return this.responseService.Response({
        message: 'Enrolls fetched successfully',
        data: enrolls,
